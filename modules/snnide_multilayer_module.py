...
class SNNIDEMultiLayerModule(nn.Module):
    ...
    def __init__(self, snn_func, snn_func_copy):
        super(SNNIDEMultiLayerModule, self).__init__()
    ...
    def forward(self, u, **kwargs):
        ...
        with torch.no_grad():
            ...
            z1_f, z1_out = self.snn_func.snn_forward(u, time_step, input_type=input_type)
            if self.training:
        ...
        if self.training:
            ...
            z1_out_ = self.snn_func.equivalent_func(z1_out, u, f_type='last')

            self.snn_func_copy.copy(self.snn_func)
            self.snn_func_copy.set_bn_mode_s('eval')

            # change the dimension of z1_out_ to be consistent with the solver
            sizes = z1_out_.size()
            B = z1_out_.size(0)
            z1_out_ = z1_out_.reshape(B, -1, 1)

            if threshold > 0:
                z1_out_ = self.Backward.apply(self.snn_func_copy, z1_out_, u, sizes, threshold, solver_type)

            # change back the dimension
            z1_out_ = torch.reshape(z1_out_, sizes)
    ...
    class Backward(Function):
        ...
        def backward(ctx, grad):
            ...
            def infer_from_vec(z, u):
                ...
                return (snn_func.equivalent_func(z_in, u, f_type='last') - z_in).reshape(B, -1, 1)
